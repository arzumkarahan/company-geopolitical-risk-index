...
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzCKakxQF-e2nkIbLhJigqCFShBNB7ycXJxmTjLFs0XfG9qpTPSTG-XoFXK923_wT8ZYg/exec"

def _submit_email(email: str, company: str, score: float, risk_level: str):
    try:
        requests.post(APPS_SCRIPT_URL, json={
            "email": email, "company": company,
            "score": score, "riskLevel": risk_level,
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if run_calc:
        # Reset email gate so each new computation requires re-confirmation
        st.session_state.export_email_confirmed = False
        st.session_state.export_email_value = ""
        missing = []
...
<div class="subtitle">Corporate Geopolitical Risk Index · 2024 Edition · Generated by CGRI Tool</div>
...
Open this file in a browser and use <b>File → Print → Save as PDF</b> to export as PDF.</div>
        ...
        # ── Email gate ───────────────────────────────────────────────────────
        if "export_email_confirmed" not in st.session_state:
            st.session_state.export_email_confirmed = False
        if "export_email_value" not in st.session_state:
            st.session_state.export_email_value = ""

        if not st.session_state.export_email_confirmed:
            st.info("Enter your email to unlock the export options.")
            email_col, btn_col = st.columns([3, 1])
            raw_email = email_col.text_input(
                "Email address", placeholder="your@email.com",
                label_visibility="collapsed"
            )
            if btn_col.button("Unlock exports", type="primary"):
                if re.match(r"^[^\s@]+@[^\s@]+\.[^\s@]+$", raw_email.strip()):
                    st.session_state.export_email_value = raw_email.strip()
                    st.session_state.export_email_confirmed = True
                    _submit_email(
                        raw_email.strip(), company_name,
                        result["final_cgri"], cat
                    )
                    st.rerun()
                else:
                    st.error("Please enter a valid email address.")
        else:
            st.success(f"✓ Exports unlocked for {st.session_state.export_email_value}")
